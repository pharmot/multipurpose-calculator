...
const kcentra = {
  ...
  processInput() {
    const self = this;
    $("#kcentra--output").val("");
    const scancode = $("#kcentra--scancode").val();
    if ( scancode.length > 0 ) {
      let allScans = "";
      if ( scancode.length % 54 === 0 ) {
        const trimmed = scancode.substring(1, scancode.length - 1 );
  ...
  makeText() {
    ...
    $(".kcentra--inputRow").each( (i, a) => {
      const myQty = parseFloat($(a).find('.qty')[0].value),
    ...
    ai = `KCentra - Lot # and Expiration\n${sumQty} vials to make ~ *** unit dose\n${ai}`;
    ...
    if ( sumUnits === 0 || sumUnits === '***' ) {
      $('#kcentra--totalUnits').html('');
    } else {
      $('#kcentra--totalUnits').html(`${sumUnits} units`);
    }
